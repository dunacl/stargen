...
void text_describe_system(planet_pointer innermost_planet, int do_gases, long int seed)
{
  ...
  printf("Star Class: %s\n", sun->starid);
  printf("Star Color: %s\n", sun->color);
  printf("Star Minimum Temperature: %d\n", sun->starmintemp);
  printf("Star Maximum Temperature: %d\n", sun->starmaxtemp);
  printf("\n");
  ...
  printf("%d\t%7.3Lf AU\t%8.3Lf EM\t%c\n",
       ...
       planet->gas_giant ? 'O' : ((planet->greenhouse_effect) && (planet->surf_pressure > 0.0)) ? '+' : ((planet->hydrosphere > .05) && (planet->hydrosphere < .95)) ? '*' : ((planet->mass * SUN_MASS_IN_EARTH_MASSES) > .1) ? 'o' : '.');
  }
  printf("\n\n\n");
  for (planet = innermost_planet, counter = 1;
     planet != NULL;
  ...
  if (do_gases && (planet->type != tGasGiant) && (planet->type != tSubGasGiant) && (planet->type != tSubSubGasGiant))
  {
    //gases?
  }
  }
...
void jsonOutput(planet_pointer innermost_planet, int do_gases, long int seed)
{
  char buffer[4096];
  ...
  jwObj_object("mass");
  jwObj_double("value", sun->mass);
  jwObj_string("unit", "Sun masses");
  jwObj_string("description", "Stellar mass");
  jwEnd();

  jwObj_object("luminosity");
  jwObj_double("value", sun->luminosity);
  jwObj_string("unit", "Sun luminosity");
  jwObj_string("description", "Stellar luminosity");
  jwEnd();

  jwObj_object("color");
  jwObj_string("value", sun->color);
  jwObj_string("unit", "color");
  jwObj_string("description", "Stellar wavelength");
  jwEnd();

  jwObj_object("class");
  jwObj_string("value", sun->starid);
  jwObj_string("unit", "class");
  jwObj_string("description", "Stellar clasification");
  jwEnd();

  jwObj_object("temperature");
  jwObj_object("minimum");
  jwObj_int("value", sun->starmintemp);
  jwObj_string("unit", "celsius");
  jwObj_string("description", "Stellar minimum temperature");
  jwEnd();

  jwObj_object("maximum");
  jwObj_int("value", sun->starmaxtemp);
  jwObj_string("unit", "celsius");
  jwObj_string("description", "Stellar maximum temperature");
  jwEnd();
  jwEnd();

  // (sun->age / 1.0E9), (sun->life - sun->age) / 1.0E9)
  jwObj_object("age");
  jwObj_object("current");
  jwObj_double("value", (sun->age / 1.0E9));
  jwObj_string("unit", "billion years");
  jwObj_string("description", "Stellar total age");
  jwEnd();

  jwObj_object("total");
  jwObj_double("value", ((sun->life - sun->age) / 1.0E9));
  jwObj_string("unit", "billion years");
  jwObj_string("description", "Stellar years to colapse");
  jwEnd();
  jwEnd();

  //min_r_ecosphere, max_r_ecosphere
  jwObj_object("ecosphere");
  jwObj_object("radius");
  jwObj_double("value", sun->r_ecosphere);
  jwObj_string("unit", "AU");
  jwObj_string("description", "Perfect life growing capable ecosphere radius");
  jwEnd();

  jwObj_object("min_radius");
  jwObj_double("value", min_r_ecosphere);
  jwObj_string("unit", "AU");
  jwObj_string("description", "Minimum life capable ecosphere radius");
  jwEnd();

  jwObj_object("max_radius");
  jwObj_double("value", max_r_ecosphere);
  jwObj_string("unit", "AU");
  jwObj_string("description", "Maximum life capable ecosphere radius");
  jwEnd();

  jwEnd(); // end object ecosphere

  jwEnd(); // end object sun
  jwEnd(); // end object solar
  /*
  SYSTEM  CHARACTERISTICS
  Stellar mass: 0.91 solar masses
  Stellar luminosity: 0.64
  Age: 1.636 billion years	(12.669 billion left on main sequence)
  Habitable ecosphere radius: 0.797 AU
  Star Class: K
  Star Color: Orange to Red
  Star Minimum Temperature: 3500
  Star Maximum Temperature: 4999
...
void csv_describe_system(FILE *file, planet_pointer innermost_planet, int do_gases, long int seed)
{
  ...
  fprintf(file,
      "%ld; '%s'; %5.3Lf; %5.3Lf; %5.3Lf; %5.3Lf; %5.3Lf; %s; %s; %d; %d\n",
      seed,
      sun->name,
      sun->luminosity,
      sun->mass,
  ...
  *buffer = '\0';
  ...
  if (do_gases && (planet->gases > 0))
  {
    ...
    if (inspired_partial_pressure(planet->surf_pressure,
                    planet->atmosphere[i].surf_pressure) > gases[index].max_ipp)
      poisonous = TRUE;
    ...
    *buffer = '\0';
    ...
    for (i = 0; i < moon->gases; i++)
    {
      ...
      if (inspired_partial_pressure(moon->surf_pressure,
                    moon->atmosphere[i].surf_pressure) > gases[index].max_ipp)
      poisonous = TRUE;
           ...
           int graphic_format)
{
     ...
     planet = planet->next_planet, counter++)
  {
    ...
    if ((planet->type == tTerrestrial) || (planet->type == tWater) || (breathability(planet) == BREATHABLE) || ((planet->surf_pressure > 0.001) && (planet->surf_temp > FREEZING_POINT_OF_WATER - 15) && (planet->surf_temp < EARTH_AVERAGE_KELVIN + 15)))
    ...
    if ((moon->type == tTerrestrial) || (moon->type == tWater) || (breathability(moon) == BREATHABLE) || ((moon->surf_pressure > 0.001) && (moon->surf_temp > FREEZING_POINT_OF_WATER - 15) && (moon->surf_temp < EARTH_AVERAGE_KELVIN + 15)))
      terrestrials_seen = TRUE;
    }
 ...
 */
  ...
  fprintf(file,
      ...
      sun->starid);
  fprintf(file,
      "<tr><td>Minimum Temp</td>\n"
      "\t<td>%d</td></tr>\n",
      sun->starmintemp);
  fprintf(file,
      "<tr><td>Maximum Temp</td>\n"
      "\t<td>%d</td></tr>\n",
      sun->starmaxtemp);
  fprintf(file,
      "<tr><td>Age</td>\n"
             ...
             FILE *file)
{
  ...
  if ((planet->dust_mass * SUN_MASS_IN_EARTH_MASSES >= 0.0006) && (planet->gas_mass * SUN_MASS_IN_EARTH_MASSES >= 0.0006) && (planet->type != tGasGiant) && (planet->type != tSubGasGiant))
  {
...

  {
    ...
    if (fabsl(planet->high_temp - planet->max_temp) > 10 || fabsl(planet->low_temp - planet->min_temp) > 10)
    {
