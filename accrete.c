...
long double collect_dust(long double last_mass, long double *new_dust, long double *new_gas, long double a, long double e, long double crit_mass, dust_pointer dust_band)
{
...
void coalesce_planetesimals(long double a, long double e, long double mass, long double crit_mass, long double dust_mass, long double gas_mass, long double stell_luminosity_ratio, long double body_inner_bound, long double body_outer_bound, int do_moons)
{
  ...
  if (((fabsl(diff) <= fabsl(dist1)) || (fabsl(diff) <= fabsl(dist2))))
  {
    ...
    if (mass < crit_mass)
    {
      if ((mass * SUN_MASS_IN_EARTH_MASSES) < 2.5 && (mass * SUN_MASS_IN_EARTH_MASSES) > .0001 && existing_mass < (the_planet->mass * .05))
      {
      ...
      if (flag_verbose & 0x0100)
        fprintf(stderr, "Moon Escapes... "
            ...
            mass * SUN_MASS_IN_EARTH_MASSES,
            (mass * SUN_MASS_IN_EARTH_MASSES) >= 2.5 ? ", too big" : (mass * SUN_MASS_IN_EARTH_MASSES) <= .0001 ? ", too small" : "");
      }
    }
    }
