...
long double luminosity(mass_ratio) long double mass_ratio;
{
  long double n;
...
long double volume_radius(mass, density) long double mass, density;
{
  long double volume;
...
long double kothari_radius(mass, giant, zone) long double mass;
int giant, zone;
{
...
long double volume_density(mass, equat_radius) long double mass, equat_radius;
{
  long double volume;
...
long double period(separation, small_mass, large_mass) long double separation, small_mass, large_mass;
{
...
int inclination(orb_radius) long double orb_radius;
{
  int temp;
...
long double escape_vel(mass, radius) long double mass, radius;
{
...
long double molecule_limit(mass, equat_radius, exospheric_temp) long double mass, equat_radius, exospheric_temp;
{
...
long double acceleration(mass, radius) long double mass, radius;
{
...
long double gravity(acceleration) long double acceleration;
{
...
long double vol_inventory(mass, escape_vel, rms_vel, stellar_mass, zone,
                          greenhouse_effect, accreted_gas) long double mass,
    escape_vel, rms_vel, stellar_mass;
...
long double pressure(volatile_gas_inventory, equat_radius, gravity) long double volatile_gas_inventory, equat_radius, gravity;
{
...
long double boiling_point(surf_pressure) long double surf_pressure;
{
...
long double hydro_fraction(volatile_gas_inventory, planet_radius) long double volatile_gas_inventory, planet_radius;
{
  long double temp;
...
long double cloud_fraction(surf_temp, smallest_MW_retained, equat_radius, hydro_fraction) long double surf_temp, smallest_MW_retained, equat_radius,
    hydro_fraction;
{
...
long double ice_fraction(hydro_fraction, surf_temp) long double hydro_fraction, surf_temp;
{
  long double temp;
...
long double eff_temp(ecosphere_radius, orb_radius, albedo) long double ecosphere_radius, orb_radius, albedo;
{
  return (sqrt(ecosphere_radius / orb_radius) * pow1_4((1.0 - albedo) / (1.0 - EARTH_ALBEDO)) * EARTH_EFFECTIVE_TEMP);
}

long double est_temp(ecosphere_radius, orb_radius, albedo) long double ecosphere_radius, orb_radius, albedo;
{
...
long double green_rise(optical_depth, effective_temp, surf_pressure) long double optical_depth, effective_temp, surf_pressure;
{
...
long double planet_albedo(water_fraction, cloud_fraction, ice_fraction, surf_pressure) long double water_fraction, cloud_fraction, ice_fraction, surf_pressure;
{
...
long double opacity(molecular_weight, surf_pressure) long double molecular_weight, surf_pressure;
{
    ...
    planet_pointer planet;
{
       ...
       count++)
  {
    ...
    if (fabsl(planet->surf_temp - last_temp) < 0.25)
      break;
  }
